...
type TicketPricingProps = {
    ticket_pricings?: { id: number; type: string; price: string; offer_quantity: string; started_at: string; ended_at: string };
    mode: 'create' | 'edit';
...
const TicketPricingForm = ({ ticket_pricings, mode, setDialogOpen }: TicketPricingProps) => {
    ...
    useEffect(() => {
        setData('period', {
            started_at: moment(startDate).format('YYYY-MM-DD HH:mm:ss'),
            ended_at: moment(endDate).format('YYYY-MM-DD HH:mm:ss'),
        });

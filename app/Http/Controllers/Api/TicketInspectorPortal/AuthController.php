<?php

namespace App\Http\Controllers\Api\TicketInspectorPortal;

use Illuminate\Http\Request;
use App\Services\ResponseService;
use Illuminate\Support\Facades\DB;
use App\Http\Controllers\Controller;
use App\Repositories\OTPRepository;
use App\Repositories\TicketInspectorRepository;
use Illuminate\Support\Facades\Auth;

class Authcontroller extends Controller
{
    public function twoStepVerification(Request $request)
    {
        $request->validate([
            'otp_token' => 'required',
            'otp_code' => 'required',
        ], [
            'otp_token.required' => 'The OTP Token is required',
            'otp_code.required' => 'The OTP Code is required',
        ]);
        DB::beginTransaction();
        try {
            (new OTPRepository)->verify($request->otp_token, $request->otp_code);
            $decrypted_data = decrypt($request->otp_token);
            $ticket_inspector = (new TicketInspectorRepository)->findByEmail($decrypted_data['email']);
            if (!$ticket_inspector) {
                throw new \Exception('The ticket inspector is not found');
            }
            $ticket_inspector->email_verified_at = now();
            $ticket_inspector->save();
            $response = [
                'access_token' => $ticket_inspector->createToken($ticket_inspector->email)->plainTextToken
            ];
            DB::commit();
            return ResponseService::success(data: $response, message: 'Successfully Logged In');
        } catch (\Exception $e) {
            DB::rollBack();
            return ResponseService::fail(message: $e->getMessage());
        }
    }

    public function resendOtp(Request $request)
    {
        $request->validate([
            'otp_token' => 'required',
        ]);
        DB::beginTransaction();
        try {
            $otp = (new OTPRepository)->resend($request->otp_token);
            DB::commit();
            $response = [
                'otp_token' => $otp->otp_token
            ];
            return ResponseService::success(data: $response, message: 'Successfully Resend OTP');
        } catch (\Exception $e) {
            DB::rollBack();
            return ResponseService::fail(message: $e->getMessage());
        }
    }

    public function login(Request $request)
    {
        $request->validate([
            'email' => 'required|email',
            'password' => 'required',
        ]);

        DB::beginTransaction();
        try {
            if (Auth::guard('ticket_inspector')->attempt($request->only('email', 'password'))) {
                $ticket_inspector = Auth::guard('ticket_inspector')->user();
                if ($ticket_inspector->email_verified_at) {
                    $response = [
                        'is_verified' => true,
                        'access_token' => $ticket_inspector->createToken($ticket_inspector->email)->plainTextToken
                    ];
                } else {
                    $otp = (new OTPRepository)->send($ticket_inspector->email);
                    $response = [
                        'is_verified' => false,
                        'otp_token' => $otp->otp_token
                    ];
                }
            } else {
                throw new \Exception('These credentials do not match our records.');
            }

            DB::commit();
            return ResponseService::success(data: $response, message: 'Successfully Logged In');
        } catch (\Exception $e) {
            DB::rollBack();
            return ResponseService::fail(message: $e->getMessage());
        }
    }

    public function logout()
    {
        try {
            auth()->user()->currentAccessToken()->delete();
            return ResponseService::success(message: 'Successfully Logged Out');
        } catch (\Exception $e) {
            return ResponseService::fail(message: $e->getMessage());
        }
    }
}
